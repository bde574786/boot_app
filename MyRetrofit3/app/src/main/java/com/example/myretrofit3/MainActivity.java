...
public class MainActivity extends AppCompatActivity {
    ...
    private void addEventListener() {
        ...
        btn2.setOnClickListener(view -> {
            service.postsList().enqueue(new Callback<List<ResponsePostDto>>() {
                @Override
        ...
        btn3.setOnClickListener(view -> {
            // 로그인 (userId --> 쉐어드프리퍼런스에 저장) (SQL Lite) (메모리 변수 저장)
        ...
        btn4.setOnClickListener(view -> {
            ReqPostDto reqPostDto = new ReqPostDto("회의1", "DB설계", 10);
            service.updatePost(reqPostDto, 10).enqueue(new Callback<ResponsePostDto>() {
                @Override
                public void onResponse(Call<ResponsePostDto> call, Response<ResponsePostDto> response) {
                    if (response.isSuccessful()) {
                        Log.d(TAG, response.body().toString());
